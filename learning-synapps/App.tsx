...
import { GraphData, GraphNode, GraphLink, AppState, SynthesisResult, NodeExplanation, SavedMap } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const storedMaps = localStorage.getItem('synapps_saved_maps');
    if (storedMaps) {
      try {
  ...
  // Update LocalStorage whenever savedMaps changes
  useEffect(() => {
    localStorage.setItem('synapps_saved_maps', JSON.stringify(savedMaps));
  }, [savedMaps]);
  ...
  const handleInitialSearch = async (topic: string) => {
    ...
    setSearchTerm('');
    setUndoData(null);
    setSavedNodes([]); // Clear current session library
    try {
  ...
  const handleSaveSession = () => {
      ...
      // Find root node label for title
      const rootNode = graphData.nodes.find(n => n.type === 'root') || graphData.nodes[0];
      const title = rootNode ? rootNode.label : 'Untitled Session';
      ...
      // Show notification
      setSaveNotification("Session Saved Successfully!");
      setTimeout(() => setSaveNotification(null), 3000);
  };

  const handleLoadSession = (map: SavedMap) => {
      setAppState(AppState.LOADING_GRAPH);
      // Need a small timeout to allow UI to reset cleanly
      setTimeout(() => {
          setGraphData(map.data);
          setSavedNodes(map.library);
          setAppState(AppState.EXPLORING);
      }, 100);
  };
  ...
  const resetApp = () => {
    ...
    setSelectedForSynthesis(new Set());
    setSearchTerm('');
    setSavedNodes([]); // Clear history on full restart (history is preserved in savedMaps)
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-[#050505] text-white font-sans selection:bg-violet-500/30">
      ...
      {graphData.nodes.length === 0 && appState !== AppState.LOADING_GRAPH && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center p-4 pointer-events-auto">
            <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-4 text-white">SYNAPPS</h1>
            ...
            {savedMaps.length > 0 && (
                <div className="mt-12 w-full max-w-2xl animate-fade-in">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <span className="text-xs uppercase tracking-widest text-gray-500">Saved Sessions</span>
                        <div className="h-px bg-white/10 flex-1"></div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[30vh] overflow-y-auto custom-scrollbar pr-2">
                        {savedMaps.map(map => (
                            <div 
                                key={map.id} 
                                onClick={() => handleLoadSession(map)}
                                className="group relative bg-white/5 hover:bg-white/10 border border-white/5 hover:border-violet-500/30 p-4 rounded-xl transition-all cursor-pointer"
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-bold text-gray-200 group-hover:text-white truncate pr-6">{map.title}</h3>
                                    <button 
                                        onClick={(e) => handleDeleteSession(e, map.id)}
                                        className="text-gray-600 hover:text-red-400 p-1 rounded hover:bg-white/10 absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              <div className="pointer-events-auto flex flex-col gap-2 w-full max-w-xs">
                  <div className="flex gap-2">
                    ...
                    <button 
                        ...
                        className="bg-black/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors flex items-center gap-2"
      ...
      {saveNotification && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in">
              <div className="bg-emerald-600/90 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2">
